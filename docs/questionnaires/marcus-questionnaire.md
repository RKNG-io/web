# Questionnaire: The Overwhelmed Owner (Marcus)

Different animal entirely. Marcus doesn't need to start or tighten up  -  he needs to **get his life back**.

---

## Flow Structure

```
1. WELCOME           -  See him, acknowledge the weight
2. THE BUSINESS      -  What he's built (respect it)
3. THE MACHINE       -  How it runs (or doesn't)
4. THE BOTTLENECK    -  Where he's stuck in the middle
5. THE COST          -  What it's costing him (life, not just money)
6. THE DREAM         -  What freedom looks like
7. THE READINESS     -  Trust, budget, commitment
8. THE MOMENT        -  The thing that would change everything
```

---

## 1. Welcome

> **You built something real.**
>
> Employees. Customers. Revenue. Responsibility.
>
> But somewhere along the way, the thing you built started running you.
>
> This isn't about working harder. It's about building the systems that let you step back  -  and have it all keep working.
>
> These questions are direct. I need to understand what's actually happening, not the version you tell people at networking events.
>
> Takes about 10 minutes. Could save you 10 hours a week.
>
> **[Let's be honest →]**

---

## 2. The Business

*Respect what they've built. Get the facts.*

### Q1: What's your business?
*(Free text  -  keep it open for variety)*

> In a sentence: what do you do, and who do you do it for?

### Q2: How long have you been running it?
*(Single choice)*

- Less than 2 years
- 2–5 years
- 5–10 years
- 10+ years

### Q3: What's the shape of the business?
*(Multi-select)*

- Single location
- Multiple locations
- Mostly online / remote
- Service-based (clients / customers come to me)
- Product-based (I sell things)
- Hybrid

### Q4: How many people work in the business?
*(Single choice)*

- Just me
- 1–3 (including me)
- 4–10
- 11–25
- 25–50
- 50+

### Q5: What's annual revenue (roughly)?
*(Single choice)*

- Under £50k
- £50k–£100k
- £100k–£250k
- £250k–£500k
- £500k–£1m
- £1m+
- Prefer not to say

### Q6: Is the business profitable?
*(Single choice)*

- Yes  -  healthily
- Yes  -  but tighter than I'd like
- Breaking even
- Not currently
- I'm honestly not sure

---

## 3. The Machine

*How does it actually run?*

### Q7: What tools are you currently using?
*(Multi-select)*

**Operations / General**
- Google Workspace (Gmail, Drive, etc.)
- Microsoft 365
- Slack / Teams
- WhatsApp (for business comms)
- Notion / Airtable / Monday
- Trello / Asana

**Bookings / Scheduling**
- Calendly / Cal.com
- Acuity
- Industry-specific (Glofox, Mindbody, Fresha, etc.)
- Manual / phone / email

**Payments / Finance**
- Stripe
- Square
- GoCardless
- Xero / QuickBooks / FreeAgent
- Invoicing manually
- Cash / bank transfer

**Marketing / CRM**
- Mailchimp / Klaviyo
- HubSpot
- Social media only
- Nothing formal

**Other**
- Industry-specific tools: ________
- Spreadsheets for everything
- Honestly, it's mostly in my head

### Q8: How many of those tools are you actually using properly?
*(Single choice)*

- Most of them  -  we're fairly systemised
- Some  -  but there's a lot of unused potential
- Few  -  we're paying for things we don't use
- Almost none  -  I don't trust the systems

### Q9: How do you know if last month was a good month?
*(Single choice)*

- Clear dashboard / reports I check regularly
- I look at the bank balance
- My accountant tells me eventually
- Gut feel
- I don't  -  not until much later

### Q10: If you disappeared for two weeks, what would break?
*(Free text)*

> Be specific. What decisions, processes, or fires only you can handle?

---

## 4. The Bottleneck

*Where is he stuck in the middle?*

### Q11: What still runs through you that shouldn't?
*(Multi-select)*

- Approving every decision
- Answering staff questions they should know
- Handling customer complaints
- Scheduling / rotas
- Ordering / inventory
- Quoting / pricing
- Invoicing / chasing payments
- Posting on social media
- Responding to enquiries
- Hiring / firing / HR issues
- "Fixing" things when they go wrong
- Everything  -  I'm the hub of the wheel

### Q12: Why does it still run through you?
*(Multi-select)*

- No one else knows how
- I don't trust anyone else to do it right
- There's no system / process documented
- It's faster to just do it myself
- I haven't had time to train someone
- I tried to delegate and it went wrong
- Control  -  I struggle to let go
- I don't know how to hand it over

### Q13: What questions do your team ask you that they should already know the answer to?
*(Free text)*

### Q14: How often are you solving the same problem more than once?
*(Single choice)*

- Constantly  -  same fires, different day
- Often  -  patterns keep repeating
- Sometimes  -  but mostly under control
- Rarely  -  we've got good systems

---

## 5. The Cost

*This is where it gets real.*

### Q15: How many hours a week are you working?
*(Single choice)*

- Under 40
- 40–50
- 50–60
- 60–70
- 70+
- I've stopped counting

### Q16: When did you last take a proper holiday  -  fully offline?
*(Single choice)*

- In the last 3 months
- 6 months – 1 year ago
- Over a year ago
- I can't remember
- I've never fully switched off since starting

### Q17: What is this costing you outside of work?
*(Multi-select)*

- Time with family / partner / kids
- Health  -  sleep, exercise, stress
- Friendships  -  I've disappeared
- Hobbies  -  things I used to enjoy
- Mental peace  -  I can't switch off
- Patience  -  I'm shorter with people I love
- Joy  -  I built this to be free, not trapped
- Nothing major  -  I'm managing okay

### Q18: Complete this sentence: "I didn't build this business to..."
*(Free text)*

### Q19: What's the thing you're most afraid to admit about how things are right now?
*(Free text, optional  -  but powerful)*

---

## 6. The Dream

*What does freedom look like?*

### Q20: If the business ran smoothly without you for a month, what would you do with that time?
*(Free text)*

### Q21: What does "success" look like now  -  not when you started, but now?
*(Single choice + free text)*

- Growth  -  I want to scale further
- Stability  -  I want it to run without chaos
- Exit  -  I want to sell it eventually
- Balance  -  I want my life back
- Pivot  -  I want to change direction
- Other: ________

### Q22: Have you ever thought about selling the business?
*(Single choice)*

- Yes  -  actively exploring it
- Yes  -  but not yet
- Sometimes  -  when I'm exhausted
- No  -  I want to keep it
- No  -  but only because no one would buy this mess

### Q23: What would need to be true for someone to buy it?
*(Free text, optional but diagnostic)*

> This question isn't about selling. It's about seeing your business as an outsider would.

---

## 7. The Readiness

*Trust, capacity, commitment.*

### Q24: Have you worked with consultants, coaches, or agencies before?
*(Single choice)*

- Yes  -  good experience
- Yes  -  mixed results
- Yes  -  waste of money, won't do it again
- No  -  never found the right one
- No  -  never needed to

### Q25: What would make you trust someone to help with this?
*(Multi-select)*

- They've run a business themselves
- They understand my industry
- They listen before prescribing
- They show me the problem before selling the solution
- They're honest, not salesy
- They can prove results
- Recommendations from people I trust
- Affordable  -  I've wasted money before

### Q26: What's your capacity to invest in fixing this?
*(Single choice)*

- Under £500  -  cautious, need to see value first
- £500–£1,500  -  ready if it's the right solution
- £1,500–£3,000  -  serious about change
- £3,000–£5,000  -  want comprehensive help
- £5,000+  -  just fix it, my time is worth more
- Not sure yet  -  depends what's possible

### Q27: What would make this feel like money well spent?
*(Free text)*

---

## 8. The Moment

*The reckoning.*

### Q28: If you could fix ONE thing in the next 30 days, what would make the biggest difference?
*(Free text)*

### Q29: What do you need to hear right now?
*(Single choice)*

- "Here's exactly what's broken and how to fix it"
- "You're not failing  -  this is normal"
- "It doesn't have to be this hard"
- "Here's the first step  -  just one thing"
- "Someone else can do this for you"
- "You've built something valuable  -  now protect it"

### Q30: Anything else I should know?
*(Free text, optional)*

---

## Closing

> **You built this.**
>
> Not everyone can. Most people never try.
>
> But the business you built doesn't have to cost you everything else.
>
> Your Reckoning is coming  -  a clear-eyed look at where the machine is breaking down, what's costing you the most, and exactly where to start.
>
> Not a sales pitch. A diagnosis. Then you decide.
>
> **Your time is now.**

---

## Data Captured → Report Inputs

| Input | Used For |
|-------|----------|
| Business profile (Q1–Q6) | Context, tailor recommendations |
| Tools in use (Q7, Q8) | "You're already paying for this" recommendations |
| Visibility (Q9) | Reporting / dashboard recommendations |
| What breaks without him (Q10) | Priority bottleneck identification |
| Bottleneck areas (Q11, Q12, Q13) | Specific fix recommendations |
| Pattern repetition (Q14) | Process documentation needs |
| Hours + holiday (Q15, Q16) | "Cost of inaction" calculation |
| Personal cost (Q17, Q18, Q19) | Emotional resonance in report |
| Freedom vision (Q20, Q21) | Frame recommendations toward their goal |
| Exit thinking (Q22, Q23) | "Business value" angle if relevant |
| Trust factors (Q24, Q25) | Tone of report, address objections |
| Budget (Q26, Q27) | Package recommendations |
| One thing (Q28) | Lead recommendation |
| What they need to hear (Q29) | Opening line of report |

---

## Conditional Logic

**Based on Q3 (shape):**
- Multiple locations → add questions about cross-site visibility, staff consistency
- Online/remote → add questions about async communication, remote team management

**Based on Q4 (team size):**
- 10+ staff → add questions about middle management, delegation layers
- 1–3 → simpler recommendations, owner-operator focus

**Based on Q7 (tools):**
- Industry-specific tool selected → add follow-up about utilisation
- "Spreadsheets for everything" → flag as priority fix
- "Mostly in my head" → flag as critical risk

**Based on Q22 (exit thinking):**
- If considering sale → add "business value" framing throughout report
- "No one would buy this mess" → powerful hook for systematisation
